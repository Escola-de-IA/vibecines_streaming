...
const FavoritesPage = () => {
  ...
  const favMovies = useMemo(() => catalog.movies.filter(m => favorites.has(m.id)), [catalog.movies, favorites]);
  const favSeries = useMemo(() => catalog.series.filter(s => favorites.has(s.id)), [catalog.series, favorites]);

  const favoritePosterTargets = useMemo(
    () => [
      ...favMovies.map(item => ({ id: item.id, title: item.title, type: 'movie' as const })),
      ...favSeries.map(item => ({ id: item.id, title: item.title, type: 'tv' as const })),
    ],
    [favMovies, favSeries],
  );
  const tmdbPosters = useTmdbPosters(favoritePosterTargets, { batchSize: 12, delayMs: 150 });
          ...
          <>
            {favMovies.length > 0 && (
              <div className="mb-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {favMovies.map(m => (
                    <ContentCard key={m.id} id={m.id} title={m.title} logo={tmdbPosters[m.id] || m.logo} group={m.group} type="movie" />
                  ))}
                </div>
              </div>
            )}
            {favSeries.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {favSeries.map(s => (
                    <ContentCard key={s.id} id={s.id} title={s.title} logo={tmdbPosters[s.id] || s.logo} group={s.group} type="series" />
                  ))}
                </div>
              </div>
            )}
